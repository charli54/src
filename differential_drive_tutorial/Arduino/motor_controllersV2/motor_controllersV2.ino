...
ros::Subscriber<geometry_msgs::Twist> subPID("ard_pid", &setPID );

//Specify the links and initial tuning parameters
double Kp=320, Ki=350, Kd=0.45;
...
TwoWheel getVelocity() {
  ...
  */
  ...
  velocity.left  = double(leftEncoderValue * 2000.0 * PI_NUMBER) /  (interval * CPR);
  velocity.right = double(rightEncoderValue * 2000.0 * PI_NUMBER) / (interval * CPR);
  
  return velocity;
}

//Subscriber to pid settings
void setPID(const geometry_msgs::Twist & pidVal) {
  myPID_left.SetTunings(pidVal.linear.x, pidVal.linear.y, pidVal.linear.z);
  myPID_right.SetTunings(pidVal.linear.x, pidVal.linear.y, pidVal.linear.z);
}
